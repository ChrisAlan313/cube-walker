...
describe('GameState', () => {
    describe('moveLeft', () => {
        it('should move the player left', () => {
            const gameState = new GameState({playerPos: {x: 0, y: 0}, gameWidth: 1, gameHeight: 1});
            gameState.moveLeft();
    ...
    describe('wrapLocation', () => {
        it('should wrap the player location horizontally', () => {
            const gameState = new GameState({playerPos: {x: 0, y: 0}, gameWidth: 4, gameHeight: 4});
            gameState.moveUp();
            gameState.wrapLocation();
            expect(gameState.playerPos.x).toBe(3);
        });
        it('should wrap the player location horizontally backwards', () => {
            const gameState = new GameState({playerPos: {x: 3, y: 0}, gameWidth: 4, gameHeight: 4});
            gameState.moveDown();
            gameState.wrapLocation();
        ...
        it('should wrap the player location vertically backwards', () => {
            const gameState = new GameState({playerPos: {x: 0, y: 3}, gameWidth: 4, gameHeight: 4});
            gameState.moveRight();
            gameState.wrapLocation();
